...
AudioMatrix::AudioMatrix(std::string config_path): config_parser(config_path)
{
    ...
    for (int i = 0; i < m_audio_matrix_config.tracks.size(); i++) {
        tracks.emplace_back(std::make_unique<Track>(m_audio_matrix_config.tracks[i], m_osc_server.get_server_thread()));
    }
    m_osc_server.start();
    initialize();
}
...
void AudioMatrix::initialize() {
    ...
    for (int i = 0; i < tracks.size(); i++) {
        output_channel_offset += tracks[i]->initialize(m_audio_matrix_config.n_input_channels, output_channel_offset);
    }
...
void AudioMatrix::prepare(HostAudioConfig host_audio_config) {
    ...
    for (int i = 0; i < tracks.size(); i++) {
        tracks[i]->prepare(m_host_audio_config);
    }
}

void AudioMatrix::process(float** in, float** out, size_t nframes) {
    for (int i = 0; i < tracks.size(); i++) {
        tracks[i]->process(in, out, nframes);
    }
}
...
std::string AudioMatrix::get_ouput_port_name(size_t channel) {
    ...
    for (int i = 0; i < tracks.size(); i++) {
        if (channel < tracks[i]->get_output_channel_offset() + tracks[i]->get_number_output_channels()) {
            name = tracks[i]->get_name();
            channel_number = channel - tracks[i]->get_output_channel_offset();
            break;
        }
    }
